...
$query = mysqli_query($conn,"SELECT asker.uid,username, useremail,gitv, qid,tags,title,sum(vote.votes)  as votes FROM question left outer join asker on question.uid=asker.uid left outer join vote on question.qid=vote.quesid group by qid, title,username order by votes desc LIMIT $offset,$page_rows");
